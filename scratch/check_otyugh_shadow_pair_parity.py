"""Parity check for sims/otyugh_shadow_pair against its Phase 0 baseline."""
...
def main() -> int:
    sim_dir = Path(__file__).parent.parent / "sims" / "otyugh_shadow_pair"
    ...
    console = Console()
    table = Table(title=f"otyugh_shadow_pair parity vs baseline: {'PASS' if report.passed else 'FAIL'}")
