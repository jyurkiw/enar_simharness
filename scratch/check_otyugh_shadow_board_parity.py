"""Parity check for sims/otyugh_shadow_board against its Phase 0 baseline."""
...
def main() -> int:
    sim_dir = Path(__file__).parent.parent / "sims" / "otyugh_shadow_board"
    ...
    console = Console()
    table = Table(title=f"otyugh_shadow_board parity vs baseline: {'PASS' if report.passed else 'FAIL'}")
