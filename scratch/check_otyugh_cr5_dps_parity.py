"""Parity check for sims/otyugh_cr5_dps against its Phase 0 baseline
(design doc 05's statistical parity methodology). Not part of the shipped
`dnd5e-sim` CLI: this sim's new-engine instance names are the creature files'
own canonical names (champion_fighter, hunter_ranger, ...), while the old
engine's ledger used the scenario's short party-slot names (fighter, ranger,
...) — see simulation.toml's docstring. Renaming here keeps that a one-off
concern instead of teaching the loader/CLI a general aliasing scheme no other
sim needs.
"""
...
RENAME = {
    ...
    "ranger": "hunter_ranger",
    "rogue": "thief_rogue",
    "cleric": "life_cleric",
    "wizard": "evoker_wizard",
}
...
def main() -> int:
    sim_dir = Path(__file__).parent.parent / "sims" / "otyugh_cr5_dps"
    ...
    console = Console()
    table = Table(title=f"otyugh_cr5_dps parity vs baseline: {'PASS' if report.passed else 'FAIL'}")
    ...
    table.add_column("p10/p50/p90 delta", justify="right")
    ...
    for col in sorted(report.columns, key=lambda c: c.column):
        ...
        pct = (f"{col.p10_delta:.1%}/{col.p50_delta:.1%}/{col.p90_delta:.1%}"
              if not col.is_rate else "-")
        table.add_row(col.column, "rate" if col.is_rate else "continuous",
                      f"{col.mean_a:.2f}", f"{col.mean_b:.2f}", delta, pct,
